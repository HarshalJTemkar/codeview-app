(function () {
  ...
  function loadFlow(project) {
    if (!project) {
      container.style.display = 'none';
      emptyState.style.display = 'block';
      emptyState.textContent = 'No project selected. Upload something first: /ui/upload';
      return;
    }

    fetch('/ui/api/flow?project=' + encodeURIComponent(project))
      .then((res) => res.json())
      .then((graph) => {
        if (!graph.nodes || graph.nodes.length === 0) {
          container.style.display = 'none';
          emptyState.style.display = 'block';
          emptyState.textContent = 'Nothing indexed yet for this project.';
          return;
        }
